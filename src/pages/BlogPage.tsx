          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogPosts.map((post) => (
              <Link 
                key={post.id}
                to={`/blog/${post.slug}`}
                className="group glass rounded-2xl overflow-hidden border border-white/60 shadow-lg hover:shadow-xl transition-all duration-300 flex flex-col"
              >
                {/* Image */}
                ...
                <div className="p-6 bg-white/40 flex flex-col flex-grow">
                  <h3 className="text-xl font-display font-bold text-bit-dark mb-2 group-hover:text-bit-lavender transition-colors">
                    {post.title}
                  </h3>
                  <p className="text-sm text-bit-dark/70 mb-4 flex-grow line-clamp-2">
                    {post.excerpt}
                  </p>
                  
                  {/* Meta */}
                  <div className="flex items-center justify-between text-xs text-bit-dark/50 pt-4 border-t border-bit-dark/10">
